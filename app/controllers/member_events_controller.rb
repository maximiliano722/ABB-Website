...
class MemberEventsController < ApplicationController
  ...
  def show
    ...
    sql_query = "SELECT * FROM attendance_logs WHERE event_id = #{@event.id}"
  ...
  def create
    ...
    respond_to do |format|
      if @event.save
        format.html { redirect_to controller: :page, action: :event_view, notice: 'Event was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @event.update(member_event_params)
        format.html { redirect_to controller: :page, action: :event_view, notice: 'Event was successfully upated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to controller: :page, action: :event_view, notice: 'Event was successfully deleted.' }
  ...
  def member_event_params
    params.require(:member_event).permit(:event_name, :date, :time, :location, :event_type, :point_value,
